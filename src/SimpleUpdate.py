...
#         :return: None
#         """
#         tensors = cp.deepcopy(self.tensors)
...
#             theta = time_evolution(ri, rj, i_neighbors, j_neighbors, lambda_k, self.dt, self.j_ij[ek], self.h_k,
#                                    self.s_i, self.s_j, self.s_k)
#             # theta.shape = (qi, i'_spin_dim, j'_spin_dim, qj)
#
#             # obtain ri', rj', lambda'_k tensors by applying an SVD to theta.
#             ri_tilde, lambda_k_tilde, rj_tilde = truncation_svd(theta, keep_s='yes', d_max=self.d_max)
#
#             # reshaping ri_tilde and rj_tilde back to rank 3 tensor.
#             ri_tilde = np.reshape(ri_tilde,
#                                   (qi.shape[0], i_physical_dim, ri_tilde.shape[1]))  # (qi, i'_spin_dim, d_max)
#             ri_tilde = np.transpose(ri_tilde, [1, 2, 0])  # (i'_spin_dim, d_max, qi)
#             rj_tilde = np.reshape(rj_tilde,
#                                   (rj_tilde.shape[0], j_physical_dim, qj.shape[0]))  # (d_max, j'_spin_dim, qj)
...
#
#     def get_tensors(self, edge, tensors, structure_matrix):
...
#             tensor = np.einsum(tensor, np.arange(len(tensor.shape)), weights[edge], [dims[i]],
