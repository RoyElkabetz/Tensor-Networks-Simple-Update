...
def plot_convergence_curve(
    simple_update_object,
    figure_name="su_simulation_plot",
    figure_size=(17, 10),
    ...
    ax2.plot(
        iteration[argmin_energy],
        energy[argmin_energy],
        "o",
        color="red",
    ...
    if save_figure:
        if simple_update_object.tensor_network.network_name is not None:
            plt.savefig(
                join(
                    simple_update_object.tensor_network.dir_path,
                    simple_update_object.tensor_network.network_name,
                )
                + ".png"
            )
        else:
