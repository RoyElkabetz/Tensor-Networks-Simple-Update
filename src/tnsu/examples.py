...
def transverse_ising_field_ground_state_experiment(
    ...
    for d_max in d_max_:
        ...
        network_name = 'TIF_' + trans_field_op + '_' + exp_name + '__' + 'h_' + str(h_k) + '__' + 'D_' + \
                       str(d_max) + '__' + 'spin_1/2'

        # create the Tensor Network object
        tn = TensorNetwork(structure_matrix=smat,
                           virtual_dim=2,
                           spin_dim=int(2),
...
def afh_ground_state_experiment(
        smat: np.array, spin: float = 0.5,
        d_max_=None, error: float = 1e-6,
        max_iterations: int = 200,
        dts=None, dir_path: str = '../tmp/networks',
        plot_results: bool = True, save_network: bool = False,
        seed: int = 42, exp_name: str = ""
) -> tuple[list[TensorNetwork], list[float]]:
    """
...
def afh_chain_spin_half_ground_state_experiment(
    ...
    return afh_ground_state_experiment(
        smat=smat, spin=spin, d_max_=d_max_, error=error, max_iterations=max_iterations, dts=dts, dir_path=dir_path,
        plot_results=plot_results, save_network=save_network, seed=seed, exp_name=exp_name
    )
...
def fhf_ground_state_experiment(
        smat: np.array, spin: float = 0.5,
        d_max_=None, error: float = 1e-6,
        transverse_field_op: str = 'x', h_k: float = 0.,
        max_iterations: int = 200,
        dts=None, dir_path: str = '../tmp/networks',
        plot_results: bool = True, save_network: bool = False,
        seed: int = 42, exp_name: str = ""
...
def fhf_pyrochlore_spin_half_ground_state_experiment(
    ...
    return fhf_ground_state_experiment(
        smat=smat, spin=spin, d_max_=d_max_, error=error, transverse_field_op=transverse_field_op, h_k=h_k,
        max_iterations=max_iterations, dts=dts, dir_path=dir_path, plot_results=plot_results,
        save_network=save_network, seed=seed, exp_name=exp_name
    )
